...
- **Engines**: sampling (LHS / Sobol / Monte-Carlo / grid), GLUE, SMC-PF, MCMC, **DE-MC** (under the legacy `dream` selector) and **ES-MDA** ensemble posteriors, Nelder-Mead / differential-evolution / **CMA-ES** optimisers, **Bayesian optimisation** (GP + Expected Improvement), NSGA-II multi-objective, Morris/Sobol sensitivity screening, AgMIP stepwise BIC/AICc selection, and GP/RF surrogate acceleration.

  Pick the estimator with `method.bayesian.engine` (`glue` | `smc_pf` | `mcmc` | `dream` | `es_mda` | `bayesopt`) or `method.optimizer.engine` (`nelder_mead` | `diffevo` | `cmaes`). The legacy `dream` selector currently runs DE-MC, not full DREAM(ZS). Rules of thumb: **CMA-ES** for a single best-fit, **DE-MC** for a multi-chain posterior on correlated coefficients, **ES-MDA** for many parameters with uncertainty in a few iterations, **Bayesian optimisation** when each DSSAT run is expensive.
...
pip install -e .[shared]  # + immutable dssatengine workspace baseline
pip install -e .[acquire] # + immutable dssatutils workspace baseline
